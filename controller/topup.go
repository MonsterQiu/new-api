...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if _, err := model.GrantInviteRebate(model.InviteRebateGrantParams{
				InviteeId:     topUp.UserId,
				SourceType:    model.InviteRebateSourceTopUp,
				...
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 邀请返利失败 trade_no=%s user_id=%d quota_to_add=%d error=%q", topUp.TradeNo, topUp.UserId, quotaToAdd, err.Error()))
			}
