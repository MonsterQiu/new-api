...
> **如何修改本文档**
>
> 直接编辑 `web/default/public/documentation/guide.md`。保存、提交并重新部署后，`/docs` 页面会自动更新。正文使用标准 Markdown，可以自由增删标题、列表、表格、引用和代码块。
...
多媒体模型的接口路径和参数与聊天模型不同。调用前先在模型详情中确认支持的端点类型。

| 能力     | 常见接口                   | 注意事项                         |
| -------- | -------------------------- | -------------------------------- |
| 图片生成 | `/v1/images/generations`   | 确认尺寸、质量和张数限制         |
| 语音合成 | `/v1/audio/speech`         | 确认音色和输出格式               |
| 语音转写 | `/v1/audio/transcriptions` | 通常需要 multipart 文件上传      |
| 视频任务 | 以模型详情页为准           | 通常为异步任务，需要查询任务状态 |

如果返回“不支持的端点”，优先检查所选模型的能力，不要反复更换 API Key。
